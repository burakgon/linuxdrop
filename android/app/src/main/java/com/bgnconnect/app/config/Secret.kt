package com.bgnconnect.app.config
...
class Secret(context: Context) {
    ...
    /** Pairing string for QR / text: bgnconnect://pair?s=<b64url>&relay=<url>. */
    fun pairingUri(): String? {
        val bytes = secretBytes() ?: return null
        val s = Base64.getUrlEncoder().withoutPadding().encodeToString(bytes)
        return "bgnconnect://pair?s=$s&relay=${Uri.encode(relayUrl)}"
    }

    /** Accepts a bgnconnect:// pairing URI or a raw hex key. Returns true on success. */
    fun importPairing(input: String): Boolean {
        val text = input.trim()
        if (text.startsWith("bgnconnect://")) {
